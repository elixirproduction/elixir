...
// CONFIGURATION
// =====================================================
const DEFAULT_WISP = "wss://wisp.mercurywork.shop/wisp/";
const WISP_SERVERS = [
    { name: "Mercurywork", url: "wss://wisp.mercurywork.shop/wisp/" },
    { name: "Anura's Wisp", url: "wss://anura.pro/wisp/" },
    { name: "Rhw's Wisp", url: "wss://wisp.rhw.one/wisp/" },
    { name: "Sanitarium", url: "wss://wisp.sanitarium.tech/wisp/" }
];
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if ('serviceWorker' in navigator) {
        ...
        const candidates = [wispUrl, ...WISP_SERVERS.map(s => s.url)].filter((v, i, a) => v && a.indexOf(v) === i);
        let connected = false;
        for (const cand of candidates) {
            try {
                await connection.setTransport("https://cdn.jsdelivr.net/npm/@mercuryworkshop/epoxy-transport@2.1.28/dist/index.mjs", [{ wisp: cand }]);
                localStorage.setItem("proxServer", cand);
                if (cand !== wispUrl) console.log("Switched to working wisp:", cand);
                connected = true;
                break;
            } catch (e) {
                console.warn("Wisp failed:", cand, e?.message || e);
            }
        }
        if (!connected) throw new Error("All wisp servers failed to connect.");
    }
